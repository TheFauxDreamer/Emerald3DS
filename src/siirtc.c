...
// in NewGameInitData -> RtcReset, the first RTC touch since the WASM||RP2350
// boot path skips RtcInit). Redirect the port to plain statics: the bit-bang
// scribbles harmlessly, reads return 0, and the RTC behaves like a dead cart
// clock (time stays zeroed; no error status since RtcInit never runs).
static vu16 sRtcGpioPort[3];
#define GPIO_PORT_DATA        (sRtcGpioPort[0])
#define GPIO_PORT_DIRECTION   (sRtcGpioPort[1])
#define GPIO_PORT_READ_ENABLE (sRtcGpioPort[2])
#else
