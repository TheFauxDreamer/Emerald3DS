...
static u8 CopySaveSlotData(u16 sectorId, struct SaveSectorLocation *locations)
{
    ...
    for (i = 0; i < NUM_SECTORS_PER_SLOT; i++)
    {
        ...
        // A garbage id (e.g. 0xFFFF from erased/corrupt flash) would index far
        // out of bounds. On the GBA that read was harmless open-bus garbage
        // (the checksum then failed); on a memory-protected host it faults.
