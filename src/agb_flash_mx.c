...
u16 EraseFlashSector_MX(u16 sectorNum)
{
    ...
    if (sectorNum >= gFlash->sector.count)
        return 0x80FF;
    return Rp2350SaveEraseSector(sectorNum);
#elif WASM
    u32 i;
    u8 *addr;
...
u16 ProgramFlashByte_MX(u16 sectorNum, u32 offset, u8 data)
{
    ...
    if (offset >= gFlash->sector.size)
        return 0x8000;
    return Rp2350SaveProgramByte(sectorNum, offset, data);
#elif WASM
    if (sectorNum >= gFlash->sector.count)
        return 0x80FF;
    if (offset >= gFlash->sector.size)
...
u16 ProgramFlashSector_MX(u16 sectorNum, u8 *src)
{
    ...
    // Real MX flash programs over an erased sector; mirror its erase-first
    // behaviour (the non-WASM path below also erases before programming).
    result = Rp2350SaveEraseSector(sectorNum);
    if (result != 0)
        return result;
    return Rp2350SaveProgramSector(sectorNum, src);
#elif WASM
    u32 i;
    u8 *dest;
