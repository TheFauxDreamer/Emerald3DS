...
{
#if RP2350
    // Mystery Gift downloads ARM code into gDecompressionBuffer and executes it.
    // Unsupported on RP2350 (no link cable, and calling a far data buffer as
    // Thumb code won't relocate on Cortex-M33). Never reached without a partner.
    (void)client;
#else
    // exec arbitrary code
