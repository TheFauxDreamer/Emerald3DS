...
3. **`3ds/host/settings.c`**: first decide if the value belongs to the console
   ...
   - **Per save:** use the `pad` byte in `struct CtrSaveSettings`, and add the
     ...
     last free byte. The next per-save value needs a larger record. A new
     follower option can use bits 4 to 7 of the `follower` byte instead
     (`REC_FOLLOWER_*`).
...
The struct is **164 bytes at v11 to v14**:
...
- Then comes the per-save table: a clock, a count, and `CTR_SETTINGS_SAVES` (8)
  ...
  other byte for the follower switch, so a v11 file loads as it is. Version 14
  uses bits 1 to 3 of that byte for WHO, BOBBING and BALL. Each bit stores the
  choice that is not the default, so a v12 or v13 record loads with the
  defaults.
...
**Seven bytes are per save**: EXP All, the level cap, the randomizer, the bag
sort, the phone-call switch, the last ball and the follower byte (the switch,
WHO, BOBBING and BALL). Each save
...
mix of check rows and button rows, `P3_ROW_Y(i)` for page 3's check rows, and
`P4_ROW_Y(i)` for page 4's mix of check rows and choice rows.
...
| Data abort with a **small or struct-sized FAR**, Read | A null pointer plus a field offset -- FAR *is* the offset, so it is 0 only when the field is the struct's first member. `offsetof` the FAR against every struct the code frees and it names the pointer outright. The naming screen was one: `MainState_Exit` frees `sNamingScreen` while the sprites and helper tasks it created are still running, and `SpriteCB_Cursor` reads `currentPage` at offset **0x1E22** every frame, which was the reported FAR exactly. Note the trap: that screen ALREADY had a `VBLANK_REQUIRE(sNamingScreen)` guard, which fixed the VBlank reader and left the `AnimateSprites()` one. Guarding readers one at a time is how these survive. A scene's MAIN callback has the same shape as its VBlank callback: the task that frees the state returns into the rest of that callback, in the same frame. `CB2_PokeStorage` read `closeBoxFlashing` at FAR **0x2C7** after `Task_ChangeScreen` freed `sStorage`, which is each exit from the PC. The fix is the one in `CB2_NamingScreen`: after `RunTasks()`, return if the pointer is NULL. Not every case is a free. `gMPlay_PokemonCry` (`src/sound.c`) is NULL until the first cry plays, and a double battle's send-out stops the last cry first. After a skipped intro, that read `ident` at FAR **0x34**. The four cry helpers there check it under `UBFIX`. `SetMovingMonPriority` (`src/pokemon_storage_system.c`) is another: `movingMonSprite` is NULL when no Pokémon is held, and each cursor move in the PC changed `oam.priority`. The bit field change reads the byte first, at FAR **0x5**. It checks the pointer under `UBFIX`. |
