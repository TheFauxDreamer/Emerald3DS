...
|---|---|---|---|
| 10 | ✅ **SHIPPED** as the battle panel (`3ds/ui/view_battle.c`): it takes the PARTY tab for the whole battle (tappable while the player chooses), with move buttons that use the move and a party row with SWITCH IN. **Touch battle controls.** Tap a move to select it; per-move effectiveness badge against the current opponent. | Gen 5+ battle bottom screen; Gen 7 **Rotom Dex** effectiveness indicator | The highest-value item on the whole list and the most DS-like. Follow `Ctr3dsQueueBattleItem` (`src/battle_controller_player.c:292`) exactly: require `gBattlerControllerFuncs[player] == HandleInputChooseAction`, reject link/frontier/recorded battles, save and restore `gActiveBattler`. The effectiveness half is Tier 1 and can ship alone. |
...
`DrawTabBar` (`3ds/ui/bottom_screen.c:691`) is `tabW = 320 / visibleCount`. Six tabs, which the bar has now with TROPHY, is 53px each; twelve would be 26px, narrower than a fingertip and narrower than the labels. The tab bar also reflows as progress flags unlock, so widths already move under the player.
...
The 6-mon party loop in the hash, which this section once counted as a win for the refactor, already runs only where the party shows: the PARTY tab and BAG's target picker (`bottom_screen.c:651`). With views, it can move into those views' own `key`, but that is tidiness, not a saving. (`UiPartyTick()` takes the tab's visibility as an argument and returns immediately off screen, which it had to once the mon icons started animating on it.)
