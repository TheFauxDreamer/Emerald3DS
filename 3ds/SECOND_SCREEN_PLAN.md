...
| 9 | ✅ **SHIPPED** as the CLOCK page of HOME (`3ds/ui/view_clock.c`). It shows the game's local time, not the 3DS clock. **Clock and counters.** Wall clock, play time, steps, Repel steps remaining, egg-hatch step estimate. | DPPt **Pokétch** digital watch + pedometer | `Ctr3dsGetClock()` (`3ds/bridge.h:435`, side-effect free; prefer it over `RtcCalcLocalTime`, which is part of the game's time-event chain), `VAR_REPEL_STEP_COUNT`, `GetGameStat(GAME_STAT_STEPS)`. |

### Tier 2: writes game state, needs the port's existing gating discipline

| # | Feature | Origin | Notes |
|---|---|---|---|
| 10 | ✅ **SHIPPED** as the battle panel (`3ds/ui/view_battle.c`): it takes the PARTY tab for the whole battle (tappable while the player chooses), with move buttons that use the move, a party row with SWITCH IN, BAG and RUN, a foe card with stat stages and conditions, and a touch pick for a send-out after a faint. **Touch battle controls.** Tap a move to select it; per-move effectiveness badge against the current opponent. | Gen 5+ battle bottom screen; Gen 7 **Rotom Dex** effectiveness indicator | The highest-value item on the whole list and the most DS-like. Follow `Ctr3dsQueueBattleItem` (`src/battle_controller_player.c:295`) exactly: require `gBattlerControllerFuncs[player] == HandleInputChooseAction`, reject link/frontier/recorded battles, save and restore `gActiveBattler`. The effectiveness half is Tier 1 and can ship alone. |
...
**Settled: LINK is a tile of HOME.** This sketch first had LINK as the sixth tab, for the Cable Club in `ROADMAP.md` Part C. TROPHY took that slot. LINK went to EXTRA page 5 instead (`3ds/ui/ui_link.c`), and when HOME landed it became a tile with no change to `UiLinkPageDraw`, `UiLinkPageTouch` or `UiLinkPageStateKey`.

It is also the precedent for the next such view: a HOME tile costs one enum line, a title, a hint and three dispatch lines, and six cells are free.

LINK also set a second precedent: a view over a page. Its trainer card view takes the whole content area, skips the page's frame and title line, and draws its own way back. It is `UI_VIEW_LINK_CARDS` on the view stack, over the LINK page.

`ROADMAP.md` Part C.3 records the same decision.

**Settled 2026-09-26: the tab bar stays.** The sketch above turns the bar into a title bar at depth > 0. The user chose to keep the tab bar instead: tapping another tab leaves the view, tapping the active tab returns it to its top, and each view keeps its own BACK (38x22 `tab_party.c:99`, 42x22 `tab_dex.c:92` and the encounters view, 56x20 BAG's CANCEL `tab_bag.c:99`, 60x22 LINK's cards `ui_link.c:63`). Quick tab switching is the point of the second screen, and a title bar would take it away while a view is open.
...
| `3ds/ui/ui_widgets.c/.h` | `UiButton` (seed `DrawButtonH`, `tab_extra.c:249`), `UiChoiceRow` (seed the `WIDE_X`/`SPD_X` button families and the `DrawCheckRow` check rows, both in `tab_extra.c`), `UiList` (seed `tab_dex.c:215 MoveCursor`, which is the better of the two existing scroll models), `UiField`. Later: `UiGrid`, `UiStatBar`, `UiPager`. **Rule: this file may not include game headers.** Anything needing `GetMonData` is view code, not a widget. |
...
The 6-mon party loop in the hash, which this section once counted as a win for the refactor, already runs only where the party shows: the PARTY tab and BAG's target picker (`bottom_screen.c:662`). With views, it can move into those views' own `key`, but that is tidiness, not a saving. (`UiPartyTick()` takes the tab's visibility as an argument and returns immediately off screen, which it had to once the mon icons started animating on it.)
...
| 2 | `ui_widgets.c` with exactly four widgets. Prove each on two existing callers on day one: `UiButton` on `tab_extra.c` and `tab_map.c:698`; `UiChoiceRow` on a button row and a check row; `UiList` on `tab_dex.c` and `tab_bag.c`, retiring bag's inferior scroll model. | Existing tabs behave identically; bag's list now keeps the selection when scrolling. |
| 3 | ✅ **Shipped, in place.** HOME: EXTRA's label became HOME, its top a 4x3 launcher of 80x64 tiles (SETTINGS, GAMEPLAY, EXTRAS, FOLLOWER, LINK, DEBUG), each opening its page as `UI_VIEW_HOME_PAGE`. The pages stayed in `tab_extra.c` rather than splitting into `view_*.c` files, and the pager on each page's top line became its title and BACK, so no page layout moved. No pager of tiles yet: six cells are free. | All EXTRA controls still reachable and still persist. |
...
| 9 | ✅ **Shipped, with switching.** **Touch move selection** (the write half of #10). Follows `Ctr3dsQueueBattleItem`: the controller gate, `gActiveBattler` saved and restored, the d-pad's own action emitted, and the engine's follow-up request answered by a hook. The move is confirmed through `HandleInputChooseMove`'s own A-press code, so targets and doubles work as on the d-pad. Link and Frontier battles are **allowed**, unlike items: the engine allows moves and switches there, and the choice is the d-pad's. The Battle Palace refuses taps. | A tapped move resolves identically to the same move chosen with the D-pad, in singles and doubles, and is refused outright in link and frontier battles. |
...
1. `DrawButtonH` moves out of `tab_extra.c` into `UiButton`. The skin plan's step 5 defines `UiButton` and folds in both local helpers, `DrawButtonH` (`tab_extra.c:249`) and `DrawBtn` (`tab_map.c:698`), plus about a dozen inline outlines; whichever plan lands first owns it, and if `ui_widgets.c` exists by then it lives there, so it covers every future view's buttons for the same edit.
