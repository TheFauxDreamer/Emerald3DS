...
to 9 are all outstanding. Re-checked against `67090b3` on 2026-09-16.
...
- **#6, the EV/IV and stat spread**, as a panel in the existing party detail
  view.
- **#1, area encounters, in part**, as a view that MAP pushes
  (`3ds/ui/view_encounters.c`, commits `c237d04`, `453c52e`, `3c3e854`). It
  lists the species of any place tapped on the map, with seen and caught marks,
  and it applies the randomizer. It does not label the method, show level
  ranges, or show a mass outbreak or the roamer.
...
Two things not on this list shipped too. A shiny-encounter notice became the
...
The obstacle is structural, not informational. The UI has no view stack (five independent modal flags, with BACK buttons in four places and three sizes), no shared widget layer (scrolling lists implemented four times, in DEX, BAG, TROPHY and the encounters view; button helpers four times), and no text clipping at all (every panel width is hand-measured against the longest known string, which cannot survive player-authored names). Adding a dozen views to that would multiply all three problems.
...
| 8 | **Type matchup chart.** Attacking type against all 17 defending types, paged by attacking type. | DPPt **Pokétch Move Tester** | `gTypeEffectiveness` (`include/battle_main.h:85`). Reuse `TypeMultiplier()` from `3ds/ui/matchup.c:34`, which already handles the `TYPE_FORESIGHT` mid-table marker correctly. |
| 9 | **Clock and counters.** Wall clock, play time, steps, Repel steps remaining, egg-hatch step estimate. | DPPt **Pokétch** digital watch + pedometer | `Ctr3dsGetClock()` (`3ds/bridge.h:376`, side-effect free; prefer it over `RtcCalcLocalTime`, which is part of the game's time-event chain), `VAR_REPEL_STEP_COUNT`, `GetGameStat(GAME_STAT_STEPS)`. |
...
`DrawTabBar` (`3ds/ui/bottom_screen.c:661`) is `tabW = 320 / visibleCount`. Six tabs, which the bar has now with TROPHY, is 53px each; twelve would be 26px, narrower than a fingertip and narrower than the labels. The tab bar also reflows as progress flags unlock, so widths already move under the player.
...
**Open question: where LINK goes.** This sketch first had LINK as the sixth tab, for the Cable Club in `ROADMAP.md` Part C. TROPHY took that slot. There are three ways out, not decided yet:
- LINK as a page of EXTRA now, and a page or tile of HOME later. Part C then does not wait for this plan.
- LINK as a HOME tile only. Part C then waits for steps 0 to 3.
- TROPHY moves into the launcher as a tile, and LINK takes the sixth tab.

`ROADMAP.md` Part C.3 records the same question.

One 80x48 BACK target replaces the hand-placed rects at 38x22 (`tab_party.c:97`), 42x22 (`tab_dex.c:91`, and the encounters view, which matched it) and 56x20 (BAG's CANCEL, `tab_bag.c:98`). Every pushed view gets a title, which matters once there are a dozen of them. It also makes the current bug structurally impossible: today a modal flag survives a tab switch, so leaving PARTY's detail view and coming back re-enters it. The flags are `sDetailOpen` (`tab_party.c:127`), `sEntryOpen` (`tab_dex.c:98`), BAG's picker `sView` (`tab_bag.c:113`), `sOpen` (`view_encounters.c:107`) and MAP's fly confirm `sConfirm` (`tab_map.c:127`).
...
| `3ds/ui/ui_widgets.c/.h` | `UiButton` (seed `tab_extra.c:161`), `UiChoiceRow` (seed the `WIDE_X`/`SPD_X` families, ~10 callers in `tab_extra.c` alone), `UiList` (seed `tab_dex.c:209 MoveCursor`, which is the better of the two existing scroll models), `UiField`. Later: `UiGrid`, `UiStatBar`, `UiPager`. **Rule: this file may not include game headers.** Anything needing `GetMonData` is view code, not a widget. |
...
- `UiClipPush/Pop/Get` in `ui_draw.c`. Nearly free: `UiFillRect` (`:58`), `UiBlit4bppTile` (`:114`), `UiBlit8bppTile` (`:170`) and `BlitGlyph` (`ui_text.c:63`) already do per-pixel bounds tests against `0..UI_W/UI_H`. They just test against the clip instead. Four one-line edits, zero extra per-pixel work.
...
The 6-mon party loop in the hash, which this section once counted as a win for the refactor, already runs only where the party shows: the PARTY tab and BAG's target picker (`bottom_screen.c:621`). With views, it can move into those views' own `key`, but that is tidiness, not a saving. (`UiPartyTick()` takes the tab's visibility as an argument and returns immediately off screen, which it had to once the mon icons started animating on it.)
...
Mitigation: mandatory prefixes, `ui_*` for shared, `tab_*` for the six tab roots, `view_*` for pushed views, and a comment in `build_objs.sh` saying why so nobody tidies the names later. Keep the directory flat; subdirectories are silently not compiled. At `67090b3` the cheatsheet documents the hazard (section 12, "Object-name collision hazard"), but `build_objs.sh` has no such comment yet. Three files do not follow the prefixes: `bottom_screen.c`, `matchup.c` and `status_tags.c`. None of them collides with a `src/` name today.
...
|---|---|---|
| 0 | `ui_view.c/.h`, registry, title bar. Pure refactor: register the six tabs as roots, convert the modals (PARTY detail, DEX entry, BAG picker, and MAP's encounters view) to pushed views, delete their BACK rects, replace `Redraw()`'s switch. | Open PARTY detail, tap BAG, tap PARTY: you get the grid, not the detail. Same for DEX entry, the BAG picker and the encounters view. |
...
| 2 | `ui_widgets.c` with exactly four widgets. Prove each on two existing callers on day one: `UiButton` on `tab_extra.c` and `tab_map.c:428`; `UiList` on `tab_dex.c` and `tab_bag.c`, retiring bag's inferior scroll model. | Existing tabs behave identically; bag's list now keeps the selection when scrolling. |
...
| 6 | **Area encounters, the rest** (#1). The species list shipped as `view_encounters.c`, and step 0 moves it onto the stack. What is left: a method selector on `UiChoiceRow`, level ranges, and the mass outbreak and roamer. It becomes the first genuine `UiList` caller among the views. | List matches the game's own tables for several maps, with the randomiser both off and on. A map with no water and no fishing shows empty methods rather than stale rows. |
| 7 | **Quick-use item ring** (#11). First write path. A second entry point into `CanUseItemNow()` + `ItemTargeting()` (`3ds/ui/tab_bag.c:148`, `:181`), which are already proven. The quick-throw strip (`ui_quickball.c`) is not this item: it offers balls, only in battle, and is not registered by the player. It is the proof that a second entry point into the bag's gates works. | Registered items work from every tab; all four gates still refuse during a battle, a script, locked field controls, or a non-overworld callback. |
| 8 | **Battle effectiveness badges** (the read-only half of #10). Per-move super-effective / resisted / immune marks in the party move list, reusing `TypeMultiplier()` from `3ds/ui/matchup.c:34`. Ships alone and is worth having on its own. Not shipped yet: the party grid has one arrow per mon (`UiMatchupOffence`, the best of its moves, and `UiMatchupRisk`), not one mark per move. | Badges agree with actual damage in a real battle, including Normal against Ghost (the `TYPE_FORESIGHT` case `matchup.c` already handles). |
