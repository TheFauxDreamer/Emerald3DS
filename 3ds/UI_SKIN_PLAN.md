...
| check row | `UiCheckBox` (`ui_draw.c`) inside `DrawCheckRow` ([tab_extra.c:279](ui/tab_extra.c#L279)), ten rows on EXTRA's pages. Not a button shape, but a control: it needs a checked and an unchecked art state, and the pressed band applies to the whole row, which is its touch target |
...
(`UiPartyRedrawAnimated`, [tab_party.c:454](ui/tab_party.c#L454)), so it moves
...
- **Clipping**, which is where a new blitter fails silently rather than loudly:
  ...
  PARTY cells ([tab_party.c:620](ui/tab_party.c#L620)) both draw panels at
...
- **Touch parity per surface.** Every control still reachable, and nothing
  ...
  exactly that reason ([tab_party.c:1190](ui/tab_party.c#L1190)).
