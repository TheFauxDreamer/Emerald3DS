...
while IFS= read -r line; do
  ...
  # Give mid2agb "$base.mid", not "$midfile". One cfg line has no extension,
  # and mid2agb would get a path that does not exist. Then the song .s is not
  # written, and the fault shows only later, as an undefined reference from
  # gSongTable at link time.
...
# The gSongTable table refers to each song in the cfg. Thus a failure here
# always gives a link error later. Show it now.
if [ "$fail" -ne 0 ]; then
